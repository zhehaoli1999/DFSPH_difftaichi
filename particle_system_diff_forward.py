...
class ParticleSystem:
    def __init__(self, config: SimConfig, arch=ti.gpu, GGUI=False):
        ...
        if self.num_rigid_bodies > 0:
            ...
            self.rigid_x = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
            self.rigid_v0 = ti.Vector.field(self.dim, dtype=float)
            self.rigid_v = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
            self.rigid_quaternion = ti.Vector.field(4, dtype=float, needs_dual=True)
            self.rigid_omega = ti.Vector.field(3, dtype=float, needs_dual=True)
            self.rigid_omega0 = ti.Vector.field(3, dtype=float)
            self.rigid_force = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
            self.rigid_torque = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
            self.rigid_mass = ti.field(dtype=float)
            self.rigid_inertia = ti.Matrix.field(m=3, n=3, dtype=float, needs_dual=True)
            self.rigid_inertia0 = ti.Matrix.field(m=3, n=3, dtype=float)
            self.rigid_inv_mass = ti.field(dtype=float)
            self.rigid_inv_inertia = ti.Matrix.field(m=3, n=3, dtype=float, needs_dual=True)
            ...
            ti.root.dense(ti.ij, (self.steps, self.num_objects)).place(self.rigid_x.dual, self.rigid_v.dual, self.rigid_quaternion.dual, self.rigid_omega.dual,
                                                                        self.rigid_force.dual, self.rigid_torque.dual, self.rigid_inertia.dual, self.rigid_inv_inertia.dual)
            ...
            self.rigid_adjust_v = ti.field(shape=(self.dim), dtype=float, needs_dual=True)
            self.rigid_adjust_omega = ti.field(shape=(3), dtype=float, needs_dual=True)

            self.rigid_adjust_v.fill(0.0)
            self.rigid_adjust_omega.fill(0.0)
        ...
        self.x_buffer = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
        self.x_0 = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
        self.v = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
        self.acceleration = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
        self.m_V = ti.field(dtype=float, needs_dual=True)
        self.m = ti.field(dtype=float)
        self.density = ti.field(dtype=float, needs_dual=True)
        self.material = ti.field(dtype=int)
        self.color = ti.Vector.field(3, dtype=int)
        self.is_dynamic = ti.field(dtype=int)

        ti.root.dense(ti.ijk, (self.steps, self.max_iter, self.particle_max_num)).place(self.v, self.v.dual)
        ...
        ti.root.dense(ti.ij, (self.steps, self.particle_max_num)).place(self.x.dual, self.x_buffer.dual, self.x_0.dual, self.acceleration.dual,
                                                                        self.m_V.dual, self.density.dual)

        
        # used as "step -1" to satisfy resort operations
        self.init_temp_x = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
        self.init_temp_v = ti.Vector.field(self.dim, dtype=float, needs_dual=True)
        ti.root.dense(ti.i, (self.particle_max_num)).place(self.init_temp_x, self.init_temp_v, self.init_temp_x.dual, self.init_temp_v.dual)
        ...
        if self.cfg.get_cfg("simulationMethod") == 4:
            self.dfsph_factor = ti.field(dtype=float, needs_dual=True)
            self.density_adv = ti.field(dtype=float, needs_dual=True)
            ti.root.dense(ti.ij, (self.steps, self.particle_max_num)).place(self.dfsph_factor, self.dfsph_factor.dual)
            ti.root.dense(ti.ijk, (self.steps, self.max_iter, self.particle_max_num)).place(self.density_adv, self.density_adv.dual)

        self.loss = ti.field(dtype=float, shape=(), needs_dual=True)
        ...
        with open(filename, 'w') as file:
            for step in range(steps):
                ...
                for r in self.object_id_rigid_body:
                    ...
                    file.write(f"x0 {self.rigid_rest_cm[r]}\n")
                    file.write(f"v {self.rigid_v[step, r]} grad {self.rigid_v.dual[step, r]}\n")
                    file.write(f"v0 {self.rigid_v0[r]}\n")
                    file.write(f"w {self.rigid_omega[step, r]} grad {self.rigid_omega.dual[step, r]}\n")
                    file.write(f"w0 {self.rigid_omega0[r]}\n")
                    file.write(f"q {self.rigid_quaternion[step, r]} grad {self.rigid_quaternion.dual[step, r]}\n")

                    file.write(f"m {self.rigid_mass[r]}\n")
