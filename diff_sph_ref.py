# This is from https://github.com/taichi-dev/taichi/blob/master/python/taichi/examples/autodiff/diff_sph/diff_sph.py
# We will regard the official diff-taichi SPH implement as a reference to our differentiable DFSPH code.
...
def init_nn_model():
    ...
    if TRAIN:
        ...
        print("training data ", training_data.shape, "test data ",
              test_data.shape)
    else:
        BATCH_SIZE = 1
        input_states = ti.field(float,
...
def initialize_target_particle(t: ti.int32, pos: ti.template(),
                               N_target_: ti.template(),
                               current_pos: ti.int32):
...
def apply_force(t: ti.int32):
    for bs, i in ti.ndrange(BATCH_SIZE, particle_num):
        if material[i] == 1:
            ...
            if F_pos[bs, t, i][0] > 0.2 and F_pos[bs, t, i][0] < 0.3 and F_pos[
                    bs, t, i][1] < 0.2 and F_pos[bs, t, i][2] > 0.2 and F_pos[
                        bs, t, i][2] < 0.3:
                indicator = (steps - t) // (steps // 2)
                F_acc[bs, t, i] = F_jet_force[
                    t, bs] + gravity + indicator * (-gravity) * 0.1
            else:
...
def update_force(t: ti.int32):
    for bs, i in ti.ndrange(BATCH_SIZE, particle_num):
        for j in range(particle_num):
            ...
            # Viscosity forces
            F_acc[bs, t, i] += viscosity_scale * mass \
                         * (F_vel[bs, t, i] - F_vel[bs, t, j]).dot(R) / (R.norm(eps) + 0.01 * H * H) / den[bs, t, j] \
                         * W_gradient(R, H)
...
def boundary_handle(t: ti.int32):
    for bs, i in ti.ndrange(BATCH_SIZE, particle_num):
        ...
        if collision_normal_length > eps:
            ...
            F_vel[bs, t, i] -= (1.0 + damping) * collision_normal.dot(
                F_vel[bs, t, i]) * collision_normal
    ...
    if TRAIN:
        ...
        for opt_iter in range(opt_iters):
            ...
            for current_data_offset in range(0, training_sample_num,
                ...
                initialize_dists()
                initialize_target_particle(0, F_pos, N_target,
                                           current_data_offset)
                fc1.clear()
                fc2.clear()
